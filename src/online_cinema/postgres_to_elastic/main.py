...
from models import FilmWork, Genre, Persons
...
def extract_with_statekeyset_ids(sql: str, state_key: str) -> list:
...
def extract(extract_data, model):
    data = []
...
def load_to_elastic_movies(extraxted_data, index: str):
...
if __name__ == "__main__":
    ...
    state_key = {'persons_filmworks_updated_date': 'persons_filmworks_updated_date',
                 'genres_filmworks_updated_date': 'genres_filmworks_updated_date',
                 'filmworks_updated_date': 'filmworks_updated_date',
                 'genres_updated_date': 'genres_updated_date',
                 'persons_updated_date': 'persons_updated_date'}
    while True:
        s = set()
        filmwork_ids = []
        bulk(elastic, load_to_elastic_movies(
            extract(extract_with_statekeyset_ids(SQL.GENRES, state_key['genres_updated_date']), Genre), 'genre'))
        bulk(elastic, load_to_elastic_movies(
            extract(extract_with_statekeyset_ids(SQL.PERSONS, state_key['persons_updated_date']), Persons), 'person'))
        for obj in [
            *
            extract_ids(
                SQL.PERSONS_FILMWORK_ID,
                extract_with_statekeyset_ids(
                    SQL.PERSONS_ID,
                    state_key['persons_filmworks_updated_date'])),
            *
            extract_ids(
                SQL.GENRES_FILMWORK_ID,
                extract_with_statekeyset_ids(
                    SQL.GENRES_ID,
                    state_key['genres_filmworks_updated_date'])),
            *
            extract_with_statekeyset_ids(
                SQL.FILMWORK_ID,
        ...
        bulk(elastic, load_to_elastic_movies(extract(ex, FilmWork), 'movies'))
        sleep(1)
